 ...
 * 
 * ● useRef : 참조 훅
 *  -> .current 프로퍼티로 전달된 인자로 초기화된 변경 가능한
 *      ref 객체를 반환하며 반환된 객체는 컴포넌트의 라이프사이클을 통해 관리
 * 랜더링에 필요하지 않을 값을 참조할 수 있는 훅
 *  -> 컴포넌트가 특정 정보를 기억하도록 하고싶지만 만약 새 랜더링을 막고싶다면
 *     ref를 사용한다.
 * 'react 포스트잇' 이라고 생각하면 된다. = 기억하고 싶은 내용을 저장할때 쓰는 state와는 다른 변수 선언.
 * useEffect와 연계해서 사용하기도 좋다.
 * 가상DOM을 활용하는것이 아닌 DOM 직접 조작( 핵심적인 내용)
 * 
 * 
 * ● useReducer : 컴포넌트에 reducer를 추가하는 React Hook
 *  -> state와 비슷하지만 좀 더 복잡한 상태관리시 사용하는 Hook
 *  -> state의 대체함수
 *  -> state의 경우는 컴포넌트 자체를 전체 통제하는 특징
 *  -> reducer는 업데이트의 로직 자체가 분리되어 있어서 컴포넌트의 영향에서 벗어날 수 있다.
 *  -> 컴포넌트 외부에 state를 업데이트 할 수 있는 코드를 적는다.
 * 
 * ● reducer vs state
 * state : 관리해야할 상태가 1개일 때, 값이 단순할 때
 * reducer : 관리할 state가 1개 이상일 때, 값이 복잡하고 프로젝트의 규모가 클 때
 *  -> 혹은 값이 유동적일 때
 * 
 * useReducer에서 쓸 수 있는 구성 요소( 함수)
 ...
 * 
 * 
 * Hook은 일반함수와 거의 비슷하긴하다.
 * 일반함수 그냥 쓰는게 나을거 같은데 굳이 써야함?
 * 
 * ● Hook의 장점
 * 1. 코드가 간결해짐
 ...
 * 
 * ● 라이프사이클( 컴포넌트의 생명주기)
 * CBD개발 방식에서는 라이프사이클이 존재
 * 컴포넌트가 생성되고 업데이트, 소멸될 때 까지의 상태를 설명( 설정)하는 메서드
 * 
 * 마운트, 업데이트, 언마운트
 * 
 * Hooks의 등장으로 클래스형 컴포넌트를 굳이 강요받지 않는다.
 * Hooks은 함수형 컴포넌트 내에서만 사용가능한가?
 *  -> Hooks의 규칙 : 최상위 레벨에서만 호출할 것
 *     (리엑트의 함수형 컴포넌트의 구조 자체가 Hooks의 호출순서에 의존하여 해당 Hooks이 관리하는 상태와의 연관성 유지 때문에)
 * 최상위 레벨 : 컴포넌트 내부에만써야하고 내부에서 제일 최상위에 넣어라
 * 
 * 
 */